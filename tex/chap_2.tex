...
Remote satellites and planetary exploration probes, such as the Juno spacecraft whose mission is to explore the Jovian environment and moons, cost in excess of \$1.1 billion USD \cite{junopresskit}, or in the case of the James Webb Space Telescope, \$8.7 billion USD \cite{jwstfaq}.
...
Equation~\ref{eq:let} shows that the LET of an incident particle depends on many parameters of the incident particle and target material. 
Correction terms are necessary to accurately represent the stopping power of ions at low energy.
...
From Equation~\ref{eq:let} the LET of an incident particle in a target material depends strongly on the effective charge state and relative velocity.
...
Stopping power curves in silicon are shown as a function of incident ion energy per unit mass in Figure~\ref{fig:stopping-power-vs-energy-per-mass} for protons, alpha particles, carbon, oxygen, and iron.
The stopping power curves shown in Figure~\ref{fig:stopping-power-vs-energy-per-mass} were calculated using the SRIM/TRIM radiation transport codes \cite{ziegler2010srim}.
The maximum value of stopping power for a given particle species is known as the Bragg peak.
The stopping power curves shown in Figure~\ref{fig:stopping-power-vs-energy-per-mass} are approximately maximum when the velocity of the incident particle energy approaches the Fermi velocity, the velocity corresponding to electrons with energy equal to the Fermi energy, $E_f$, of the target material.
...
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{stopping_vs_energy_mass.pdf}
    \end{center}
...
The energy lost to the target material through direct ionization generates \emph{e-h} pairs in semiconductor materials. 
...
The CSDA range is obtained by integrating the reciprocal of the total stopping power from the Bethe-Bloch equation, Equation~\ref{eq:let}, with respect to the incident ion energy.
\begin{equation}
    \label{eq:csda-range}
    R(E) = \int_{E_{abs}}^E \frac{dE'}{S(E')}
\end{equation}
Equation~\ref{eq:csda-range} describes the CSDA range calculation, where $R(E)$ is the range of a particle with energy $E$, $S(E)$ is the stopping power as described by Equation~\ref{eq:let} and $E_{abs}$ is the energy at which the particle is assumed to be absorbed (i.e. at rest) within the target material.
It should be noted that Equation~\ref{eq:csda-range} is only valid for the energy range and conditions where the modified Bethe--Bloch equation is also valid.
...
The CSDA range curves shown in Figure~\ref{fig:range-vs-energy-per-mass} were calculated using the SRIM/TRIM radiation transport codes \cite{ziegler2010srim}.
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{range_vs_energy-mass.pdf}
    \end{center}
...
With the observation of low-energy proton- and muon-induced SEUs the SRAMs fabricated in sub-65~nm technology nodes show trends of increased device SEU sensitivity to ionizing radiation that does not appear to be slowing.
...
By comparison, the proton mass is 938.23~MeV/c$^2$ and the muon mass is 105.7~MeV/c$^2$, making the electron approximately 1836 and 206 times lighter than other common singly-charged particles.
...
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{delta-ray-scattering-salvat.png}
    \end{center}
...
A cartoon of an inelastic scattering event can be seen on the right hand side of Figure~\ref{fig:electron-scattering-elastic-inelastic}. 
...
The second important interaction mechanism for electrons in this work is the elastic scattering of incident electrons with atomic nuclei.
...
A cartoon of an elastic scattering event is shown on the left hand side of Figure~\ref{fig:electron-scattering-elastic-inelastic}.
...
These large angle deflections make approximating the range of electrons in matter through methods like the CSDA range particularly difficult at low energies (less than a few 10s of keV).
...
\end{equation}
In Equation~\ref{eq:elec-let-superpos}, $S_{coll}(E)$ and $S_{rad}(E)$ are the stopping power contributions from collisions and radiative processes respectively.
These additional energy loss processes do not occur for heavier particles (at least until \emph{extremely} high energies) and are important for understanding electron transport, and have consequences for electrons with energies in excess of 10~MeV in silicon.
Radiative energy loss processes will be extremely important considerations when evaluating the consequences of electrons in the space-radiation environment.
This dissertation primarily considers the contribution of inelastic and elastic scattering interactions between the incident electron and the target material.
With appropriate modifications to the Bethe-Bloch formula of Equation~\ref{eq:bethe-bloch-corr-ion} it can be shown that the maximum single electron--electron scattering event energy loss is one half of the initial electron energy, $E_i/2$ \cite{segre1964nuclei,1982spre.reptR....B,Salvat:ue}.
Electrons are therefore capable of depositing large amounts of energy within small spatial regions.

\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{electrons-csda-range.png}
    \end{center}
...
The range of energetic electrons in a target material can be approximated using Equation~\ref{eq:csda-range}.
The approximate CSDA range of electrons (and positrons) is shown in Figure~\ref{fig:csda-range-electrons}.
Below energies of approximately 1~MeV, electrons follow a similar trend as the heavy ions shown in Figure~\ref{fig:range-vs-energy-per-mass}, where the incident particle range increases with increasing energy.
...
Figure~\ref{fig:gamma_energy_vs_mat_z} shows the energy dependence of the three dominant physical processes for photons incident in material.
\begin{figure}[tb]
    \centering
...
Figure~\ref{fig:photoabsorption} shows a cartoon description of a photoabsorption event from \cite{alpen1997radiation}.
...
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{photoabsorption.pdf}
    \end{center}
...
A diagram of a Compton scattering event is shown in Figure~\ref{fig:compton_scattering_diagram2}.
...
\end{equation}
where $\hbar \omega$ is the energy of the incident photon, $m_{e}c^2$ is the electron rest energy, and $\theta$ is the scattering angle of the photon as seen in Figure~\ref{fig:compton_scattering_diagram2}.

Equation~\ref{eq:compton_energy_transfer} shows thats for small scattering angles (where $\theta \approx 0$) very little energy is transferred to the generated recoil electron.
...
The total attenuation cross-section, $\mu$, can be expressed as the superposition of the attenuation cross-section for each physical process shown in Figure~\ref{fig:gamma_energy_vs_mat_z}.
...
The total attenuation cross section, $\mu$, (in units of cm$^2$/g) versus incident photon energy in silicon is shown in Figure~\ref{fig:atten_cs_in_si}.
...
The absorption edge corresponding to the $L_1$, $L_2$, and $L_3$-shells in silicon occur at 149.7, 99.8, and 99.2~eV, respectively.
\begin{figure}[tb]
    \begin{center}
...
\end{equation}
where $N_0(E)$ is the initial number of photons with energy $E$, $\mu(E)/\rho$ is the energy-dependent mass attenuation coefficient (obtained from Figure~\ref{fig:atten_cs_in_si}), and $\rho x$ is the mass thickness of the target material.
Equation~\ref{eq:beer_lambert} provides a straight forward method for calculating the attenuation of photons and can be used to determine the energy absorbed within a specific range of the target material.
...
The basic topology of a standard six transistor (6T) SRAM can be seen in Figure~\ref{fig:SRAM_Cell}. 
...
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{haran_snm_butterfly_curves.pdf}
    \end{center}
...
Figure~\ref{fig:22nm_soi_sram_butterfly_curve_snm} shows the transfer characteristics, also known as butterfly curves, for a functional 22~nm SOI SRAM from \cite{Haran:2008ta}.
...
Here $V_{in}$ is arbitrarily chosen to be the data state of $Q$ in Figure~\ref{fig:SRAM_Cell} and $V_{out}$ represents the output value $\overline{Q}$. 
...
The SNM of an SRAM cell is the side-length, given in millivolts, of the largest inscribed square that fits between the $V_{in}$/$V_{out}$ transfer characteristics of Figure~\ref{fig:22nm_soi_sram_butterfly_curve_snm}.
Exceeding the SNM for an SRAM cell results in a change of information state.
Three sets of transfer characteristics are shown in Figure~\ref{fig:22nm_soi_sram_butterfly_curve_snm}, corresponding to supply voltages of 0.9~V (blue curves), 0.8~V (pink curves), and 0.7~V (green curves).
Figure~\ref{fig:22nm_soi_sram_butterfly_curve_snm} shows the SNM of an SRAM cell depends on the supply voltage, with lower $V_{DD}$ corresponding to a smaller SNM.
...
Carriers generated in field free regions either recombine or diffuse to nearby regions where they are collected by electric fields, contributing to transients in nearby semiconductor nodes.
...
As an \emph{example}, Figure~\ref{fig:qcrit_concept} demonstrates the concept of critical charge, which is defined as the charge required to induce an upset in an SRAM cell \cite{massengill:1993sc}.
Two transients are shown in Figure~\ref{fig:qcrit_concept} with different amounts of total charge being collected and the resulting transient on the output of the off-state transistor in an SRAM cell.
The top image of Figure~\ref{fig:qcrit_concept} shows a transient corresponding to 0.23~pC of charge collection within the SRAM cell.
...
In the bottom image of Figure~\ref{fig:qcrit_concept}, the transient shown corresponds to 0.25~pC of charge collected in the SRAM cell. 
...
However, Figure~\ref{fig:qcrit_concept} is intended solely to convey the concept of critical charge, which is defined in this dissertation as a single valued metric for determining the energy deposition threshold for the onset of errors in an SRAM cell.
We will see that the magnitude of charge collection shown in Figure~\ref{fig:qcrit_concept} is extremely large when compared to the critical charge for sub-65~nm technology nodes as discussed in Chapters~\ref{cha:experimental_investigation_of_electron_induced_seus} and \ref{ch:simulation_of_electron_induced_seus}.
...
In a similar fashion, Buehler \emph{et al.} used this SPICE simulation technique and analysis to show the SRAM cell critical charge dependence on supply voltage, those results are shown in Figure~\ref{fig:qcrit_vdd_dep} \cite{buehler1990alpha}.
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{buehler_qcrit_vdd_dep.pdf}
    \end{center}
...
This trend continues until a large discrepancy between applied and nominal supply voltage results in SRAM cell instability, causing spontaneous errors in the memory array as shown by the change in slope of Figure~\ref{fig:qcrit_vdd_dep} around 1.9~V.
...
While the magnitude of critical charge in Figure~\ref{fig:qcrit_vdd_dep} is large compared to SRAMs fabricated in sub-65~nm technology nodes, the conceptual discussion above is still valid and serves as an informative case study.
...
In~\cite{barak2004use}, Barak \emph{et al.} used alpha particles to show the SEU cross-section bias dependence on supply voltage, those results can be seen in Figure~\ref{fig:seu_vdd_dep}.
\begin{figure}[tb]
    \begin{center}
...
\end{figure}
The SEU cross-sections shown in Figure~\ref{fig:seu_vdd_dep} exhibit an extremely complicated dependence on applied bias.
In some supply voltage regions, the SEU bias dependence shows a clear exponential trend, while in others it appears more linear.
It is clear, however, that for decreasing supply voltage the SEU cross-section is a monotonically increasing function.

Additionally, Figure~\ref{fig:seu_vdd_dep} shows a dependence on the incident ion LET.
...
Due to the low onset voltage for more lightly ionizing particles, this feature may be reduced or entirely absent from the SEU cross-section versus supply voltage figures as shown by the lower LET alphas (5.49~MeV) in Figure~\ref{fig:seu_vdd_dep}.
...
\end{figure}
In \cite{Sierawski:2011tc}, Sierawski attempted to estimate the critical charge for several current- and next-generation technology nodes, the resulting calculations can be seen in Figure~\ref{fig:qcrit_trends}.
...
Figure~\ref{fig:qcrit_trends} shows an overall decrease in critical charge with decreasing technology node feature size.
...
Several studies have also investigated the fidelity of electron, or $\delta$-ray, induced SEUs with mixed conclusions.
...
The generation and interactions of $\delta$-rays in a heavy-ion environment will be discussed in Section~\ref{sec:evaluating_track_structure}.
...
The primary effects due to incident protons on microelectronics in the space radiation environment have historically been DD, TID, and SEUs caused by nuclear spallation reactions \cite{reed1994implications,reed1997heavy,reed2002evidence,Reed:2007vz}.
...
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=6in]{sram_cell_failures_proton_angle.pdf}
    \end{center}
...
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{proton_upset_cs_vs_energy.pdf}
    \end{center}
...
Figure~\ref{fig:seu_cs_vs_p_energy} shows the proton SEU cross-section as a function of incident proton energy for 45~nm and 65~nm SRAMs.
...
As the incident proton energy is reduced ($<$5~MeV) the measured upset cross-section begins to increase, corresponding to protons with sufficient LET to upset the device under test.
...
This is due to a corresponding reduction in the range of incident protons and prevents them from transporting into the sensitive region of the device with sufficient energy to impact the SRAM cells nominal operation.

Closer study of Figure~\ref{fig:seu_cs_vs_p_energy} shows that multiple supply voltage conditions were used while performing the initial low-energy proton experiments.
...
A plot showing the SEU cross-section dependence on applied bias, in arbitrary units, can be seen in Figure~\ref{fig:seu_cs_vs_appl_bias_protons} for 1 and 1.5~MeV protons (also more energetic alpha particles are shown).
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{65nm_sram_vdd_dep.pdf}
    \end{center}
...
Recalling the discussion regarding Equation~\ref{eq:qcrit}, the critical charge of elementary 6T-SRAM cell should depend on the supply voltage \cite{buehler1990alpha,roth1993monitoring,barak1999scaling}.
In Figure~\ref{fig:seu_cs_vs_appl_bias_protons}, the bias dependence manifests as a increase in the normalized failure rate for decreasing supply voltage on the device.
...
The values of critical charge from \cite{Heidel:2006tp,Rodbell:2007vl} are consistent with the estimations for SRAMs in the 65~nm technology node made by Sierawski and shown in Figure~\ref{fig:qcrit_trends}.
...
For many years, the presence of atmospheric neutrons and alpha particles (largely the result of packaging impurities) represented the primary sources of soft errors at the terrestrial level.
...
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{stopping_power_proton_muon_sier_diss.pdf}
    \end{center}
...
Figure~\ref{fig:stopping_power_protons_muons} shows the stopping power, or mass stopping power, the rate of energy loss per unit path length, for a variety of particle species.
...
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=5in]{muon_upsets_tech_generation.png}
    \end{center}
...
\end{figure}
Figure~\ref{fig:muon_upsets_energy_dep} plots the muon-induced upset cross-section as a function of incident muon energy.
The characteristics of Figure~\ref{fig:muon_upsets_energy_dep} are quite similar to low-energy proton SEU cross-sections.
There is a flat region corresponding to higher energy muons and as the incident muon energy is reduced there is an abrupt increase in the SEU cross-section.
...
Figure~\ref{fig:muon_upsets_bias_dep} shows the SEU response of a 65~nm SRAM as a function of supply voltage for incident muons with average energy of approximately 400~keV. 
...
As discussed regarding Equation~\ref{eq:qcrit} and the bias dependence of low-energy protons in Section~\ref{sub:low_energy_proton_induced_seus}, reduction in supply voltage results in a corresponding reduction in critical charge.
Consequently, reduction in critical charge with decreasing supply voltage makes the SRAM test chip vulnerable to a wider range of incident muon energies, which explains the bias dependence shown in Figure~\ref{fig:muon_upsets_bias_dep}.
\begin{figure}[htbp]
    \begin{center}
        \includegraphics[width=6in]{muon_upsets_vdd_dep.pdf}
    \end{center}
...
The main goal of this dissertation is to expand on the observation of upsets due to lightly ionizing, singly-charged particles (low-energy protons and muons) and investigate whether SRAMs fabricated in modern, sub-65~nm technology nodes exhibit SEU sensitivity to ionization from energetic electrons.
...
Fleetwood \emph{et al.} published a study seeking to quantify the ``worst-case'' SRAM radiation response by evaluating common bias combinations for the transistors comprising an SRAM cell. 
In \cite{Fleetwood:1987cfa}, the bias conditions are evaluated to a total dose of 1~Mrad(Si) and the results can be seen in Figure~\ref{fig:sram_cell_imb_nmos_vth}.
...
For consistency, the \emph{1} state of the SRAM cell is defined to be when Q is low, that is when the transistor M3 of Figure~\ref{fig:SRAM_Cell} is ``on''.
Figure~\ref{fig:sram_cell_imb_nmos_vth} indicates that the devices irradiated in the ``off'' state experience a larger threshold voltage shift than those irradiated in the ``on'' state.
...
Figure~\ref{fig:sram_cell_imb_nmos_vth} shows that the accumulation of interface traps continues even after irradiation, resulting in large, positive threshold voltage shifts for each bias condition.

\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{fleetwood_nmos_irrad_imbalance.pdf}
    \end{center}
    \caption[$\Delta V_{th}$ versus time, for \emph{n}-channel transistors irradiated to 1.0~Mrad(Si) and annealed at 25$^\circ$C.]{$\Delta V_{th}$ versus time, for \emph{n}-channel transistors irradiated to 1.0~Mrad(Si) and annealed at 25$^\circ$C \cite{Fleetwood:1987cfa}.}
...
As defined in Equation~\ref{eq:vth_imb}, a positive value of $V_{imb}$ implies a preferred \emph{1} state for the SRAM cell.
Conversely, a negative value of $V_{imb}$ implies a preferred \emph{0} state for the cell.

\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=6in]{fleetwood_nmos_irrad_imbalance_cases.pdf}
    \end{center}
    \caption{SRAM cell imbalance versus radiation and anneal time, based on the data of Figure~\ref{fig:sram_cell_imb_nmos_vth}.}
    \label{fig:sram_cell_imb}
\end{figure}

A plot of threshold voltage imbalance versus irradiation and annealing time from \cite{Fleetwood:1987cfa} can be seen in Figure~\ref{fig:sram_cell_imb} for conditions where the cell is initially in a \emph{1} state during irradiation and the cell state is either retained, rewritten, or power is removed from the cell during annealing.
Initial reports indicated that the SRAM cell was ``imprinted'' and the preferred state exclusively became that which it was irradiated under, however, Figure~\ref{fig:sram_cell_imb} shows an initial preference for the \emph{0} state when the programmed pattern was the \emph{1} state. 
...
These bias conditions correspond to the largest positive threshold voltage shift in M1 and the least positive threshold voltage shift in M3, as shown in Figure~\ref{fig:sram_cell_imb_nmos_vth}.
...
The impact of charge build up in STI on transistor \emph{I-V} characteristics is shown in Figure~\ref{fig:tid_sti_leak}.
\begin{figure}[tb]
    \begin{center}
        \includegraphics[width=5in]{barnaby_sti_leakage_tid.pdf}
    \end{center}
...
Yao \emph{et al.} used Monte Carlo simulations to infer the change in inverter transfer characteristics as a result of TID in a 90~nm technology node, which can be seen in Figure~\ref{fig:snm_shift_tid_yao}.
...
The transfer characteristics shown in Figure~\ref{fig:snm_shift_tid_yao} are drastically different from the symmetric response discussed previously regarding the 22~nm node and shown in Figure~\ref{fig:22nm_soi_sram_butterfly_curve_snm}. 
Figure~\ref{fig:sram_swt_pnts_yao} shows the pre- and post-irradiation SRAM cell switching points for the bit-line and bit-line complement.
...
\end{figure}
Figure~\ref{fig:photocurrents_sram} plots the photocurrent produced at the drain nodes and \emph{p}-well within an SRAM cell resulting from transient irradiation corresponding to a dose-rate of 5~Grad(Si)/sec.
...
Figure~\ref{fig:photocurrents_sram} shows that errors occuring in a single SRAM cell are unlikely to be a local effect, since the magnitude of transients corresponding to the drain nodes within the SRAM cell are significantly smaller than the well photocurrent.
...
\end{figure}
An example of the equivalent circuit diagram of an SRAM cell including interconnect resistances for $V_{DD}$ and $V_{SS}$ is shown in Figure~\ref{fig:array_inter_resist}.
...
\end{figure}
This can be seen in Figure~\ref{fig:railspan_coll_bit_map} where the lower left corner of the presented bit-map corresponds to SRAM bit locations furtherest from the the $V_{SS}$ supply line.
...
A plot of daily solar proton fluence can be seen in Figure~\ref{fig:proton_fluence_vs_year}.
...
Figure~\ref{fig:spe-geo-worst-day-100-mils} shows the particle flux spectra from CREME96 for a geosynchronous orbit during ``worst-day'' conditions with 100~mils of aluminum shielding \cite{tylka1997creme96}.
...
The relative abundance of elements through Z=28 is shown in Figure~\ref{fig:gcr-abundance-vs-atomic-num}.
\begin{figure}[htbp]
    \begin{center}
...
The impact of solar activity on the differential energy spectra of common elements in the GCR environment is shown in Figure~\ref{fig:gcr-fluence-vs-energy}.
Together, the combination of solar particles and the GCR environment make up the interplanetary radiation environment.
The region of consideration for particles originating from these sources ranges from geosynchronous orbits to the free space between planetary systems.
...
The initial AE-8 and AP-8 models allowed designers to account for TID, SEE, DD, and ESD effects.
...
Contour plots of the particle populations for electrons and protons can be seen in Figures~\ref{fig:earth-trapped-electrons} and \ref{fig:earth-trapped-protons}, respectively.
\begin{figure}[tb]
    \begin{center}
...
The differential flux of electrons as a function of time is shown in Figure~\ref{fig:elec-geo-flux-vs-year}.
Figure~\ref{fig:elec-geo-flux-vs-year} shows that for low-energy electrons ($<$1~MeV) electron flux at geostationary orbits have been relatively constant over time and do not vary significantly with solar activity.
This is not the case for high-energy($>$1~MeV) electrons, which vary as a function of time and solar activity.
Trapped electrons and protons exhibit two high-flux regions, an inner and outer radiation belt, separated by a low flux slot region.
...
The outer radiation belts exhibit a more significant dependence on solar activity varying in both energy and flux by several orders of magnitude day to day.
...
The differential flux spectrum as a function of distance, given in Jovian radii, is shown in Figure~\ref{fig:gire2-diff-flux} with line plots indicating the flux of electrons of a given energy.
The energy spectrum of trapped electrons in the Jovian environment ranges from approximately 10~keV to 100~MeV.
...
Differential flux spectrum for electrons in the Jovian and Europa environment phase of the Juno spacecraft mission to the Jupiter planetary system can be seen in Figure~\ref{fig:jovian-int-flux-spectrum}.
...
\begin{figure}[htbp]
    ...
    \caption[The spatial distribution of ionization energy in emulsion for incident particles with differing relative velocity. These calculations, based on Katz theory, describe the average dose deposited as a function of radial distance, $t$, from the incident ion track.]{The spatial distribution of ionization energy in emulsion for incident particles with differing relative velocity \cite{Kobetich:1968im,Katz:1968fo,Katz:1969uo}. These calculations, based on Katz theory, describe the average dose deposited as a function of radial distance, $t$, from the incident ion track.}
    \label{fig:katz_dose_in_water}
\end{figure}

Figure~\ref{fig:katz_dose_in_water} shows the radial dose profile in emulsion for several incident ion energies \cite{Kobetich:1968im,Katz:1968fo,Katz:1969uo}. 
...
For small values of incident ion energy, Figure~\ref{fig:katz_dose_in_water} shows the radial dose deposited is larger near the core of the ion track. 
As $\beta$ increases, Figure~\ref{fig:katz_dose_in_water} shows that the dose near the ion track core decreases, while the corresponding maximum radial distance energy is deposited increases.
...
In Figure~\ref{fig:1988_stapor_cu_ions}, \emph{e-h} pair density is plotted as a function of radial distance from the ion trajectory at a depth of 1~$\mu$m in silicon.
Figure~\ref{fig:1988_stapor_cu_ions} illustrates the difference in the resulting charge generation profiles for two ions with similar LETs but differing energies \cite{Stapor:1988ws}. 
\begin{figure}[htbp]
    \centering
...
The track radius of ionizing radiation events is large compared to the spacing of adjacent transistors and pitch of neighboring SRAM cells in a 22~nm technology node, shown in Figure~\ref{fig:ibm_22nm_trans_sram_spacing}.
This implies that modern technology nodes have scaled to a point where $\delta$-ray energy deposition events may be of sufficient magnitude and interact frequently enough to become a reliability concern.
...
It is therefore necessary to have a thorough understanding of the potential implications of $\delta$-rays on these devices in order to understand the SEU response of SRAMs fabricated in technology nodes that exhibit small critical charge.
